...
// The content script should already be injected by the manifest
  ...
  chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
    if (tabs[0] && tabs[0].url && tabs[0].url.includes('netsuite.com')) {
      ...
      chrome.scripting.executeScript({
        ...
        // Fix image references after receiving data
    setTimeout(() => {
      chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
        if (tabs[0] && tabs[0].url) {
          const baseUrl = tabs[0].url.substring(0, tabs[0].url.indexOf('/', 9));
          debugLog('Fixing image references with base URL: ' + baseUrl);
          
          // Fix image sources in the container
          $('#container img').each(function() {
            const src = $(this).attr('src');
            if (src && !src.includes('chrome-extension') && !src.includes('dkhjafbce')) {
              $(this).attr('src', baseUrl + '/' + src);
              debugLog('Fixed image: ' + src);
            }
          });
        }
      });
...
// The existing getScripts function remains largely unchanged
var scriptsList = [];
